...
boot_cor <- function(x, y, boot.reps, prob = 0.95) {
  ...
  boot.results <- boot(data = cbind(x, y), statistic = boot.cor, R = boot.reps)
  
  # Calculate a confidence interval
  CI <- quantile(boot.results$t, probs = c(1 - prob, prob))
  
  # Assemble list
  results.out <- data.frame(r_hat = boot.results$t0, r_sd_hat = sd(boot.results$t), CI_lower = CI[1], CI_upper = CI[2])
